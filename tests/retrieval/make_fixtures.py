...
def main() -> None:
    ...
    for d in documents:
        doc = load_document(FIXTURES_DIR / f"{d['slug']}.md")
        sections = build_sections(doc)
        chunks = build_chunks(sections)
        d["dropped_empty_blocks"] = doc.dropped_empty_blocks
        d["incomplete_pages"] = doc.incomplete_pages
        d["section_count"] = len(sections)
        d["chunk_count"] = len(chunks)

    expected = {
        "_note": (
            "Generated by make_fixtures.py from the fixtures it writes — never hand-"
            "edited. Block/section/chunk counts come from the real kbfiles/sections/"
            "chunk implementation, so this file cannot drift from the fixtures."
        ),
    ...
    for d in documents:
        print(
            f"{d['slug']}: {d['block_count']} blocks, {d['section_count']} sections, "
            f"{d['chunk_count']} chunks, {d['chars']} chars"
        )
